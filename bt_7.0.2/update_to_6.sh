...
select_node(){
    ...
    download1_Url=https://raw.githubusercontent.com/lhpmain/btpanel_crack/master/bt_7.0.2
    public_file=/www/server/panel/install/public.sh
    if [ ! -f $public_file ];then
        download_file $public_file $download1_Url/6.0/public.sh
    fi
...
install_python(){
	curl -Ss --connect-timeout 3 -m 60 $download1_Url/6.0/pip_select.sh|bash
	...
	if [ -f $pyenv_path/pyenv/bin/python ];then
		...
		if [ "$is_package" = "" ];then
			wget -O $pyenv_path/pyenv/pip.txt $download1_Url/6.0/pip.txt -T 5
	...
	wget -O $python_src $download1_Url/6.0/up/Python-${py_version}.tar.xz -T 5
	...
	cd ~
	rm -rf $python_src_path
	wget -O $pyenv_path/pyenv/bin/activate $download1_Url/6.0/up/activate.panel -T 5
	wget -O $pyenv_path/pyenv/pip.txt $download1_Url/6.0/up/pip.txt -T 5
...
update_panel(){
    wget -T 5 -O /tmp/panel.zip $download1_Url/6.0/up/LinuxPanel-${version}.zip
    ...
    if [ "${check_bt}" = "" ];then
        rm -f /etc/init.d/bt
        wget -O /etc/init.d/bt $download1_Url/6.0/up/bt7.init -T 20
        chmod +x /etc/init.d/bt
    fi
    rm -f /www/server/panel/*.pyc
    rm -f /www/server/panel/class/*.pyc
    if [ ! -f $setup_path/server/panel/config/config.json ];then
        wget -T 5 -O $setup_path/server/panel/config/config.json $download1_Url/6.0/up/config.json
        wget -T 5 -O $setup_path/server/panel/config/dns_api.json $download1_Url/6.0/up/dns_api.json
    fi
