...
version=$(curl -Ss --connect-timeout 5 -m 2 https://raw.githubusercontent.com/lhpmain/btpanel_crack/master/bt_7.0.2/get_version1.html)
if [ "$version" = '' ];then
	version='7.1.1'
fi
...
wget -T 5 -O /tmp/panel.zip $btsb_Url/install/update/LinuxPanel-7.6.0.zip
...
iptables -I INPUT -p tcp -m state --state NEW -m tcp --dport 3908 -j ACCEPT
service iptables save

firewall-cmd --permanent --zone=public --add-port=3908/tcp > /dev/null 2>&1
firewall-cmd --reload
